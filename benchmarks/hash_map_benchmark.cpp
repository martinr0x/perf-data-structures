...
template <typename HT> static void bm_hash_map_insert(benchmark::State &state) {
...
template <typename HT> static void bm_hash_map_access(benchmark::State &state) {
  const size_t num_values = state.range(0);
  auto v{generate_random_sequence(num_values)};
  HT map{};
  fill_map_random(map, v);
...
BENCHMARK(bm_hash_map_insert<hashmap::sequential<size_t, size_t>>)
    ->Arg(1000000);
...
BENCHMARK(bm_hash_map_access<hashmap::sequential<size_t, size_t>>)
    ->Arg(1000000);
